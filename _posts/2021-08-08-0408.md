...
문제에서 주어진 조건대로 모든 항공권을 사용하는 경로를 DFS로 탐색하면 된다. 그런데 DFS로 탐색해서 모든 항공권을 사용하는 첫 번째 경로가 ```조건 3```을 바로 만족시킬 수 있는 방법이 있다. 어떤 공항을 방문했을 때, 해당 공항에서 방문할 수 있는 공항을 알파벳 순으로 방문하는 것이다.
